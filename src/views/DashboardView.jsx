  ...
  if (!appointments.length) return (
    <div className="p-6 text-center flex flex-col items-center justify-center h-full app-screen pb-28">
      <div className="pro-card p-8 w-full">
        <div className="w-20 h-20 bg-gradient-to-br from-cyan-500 to-emerald-500 rounded-lg flex items-center justify-center mb-6 mx-auto shadow-lg shadow-cyan-500/20"><Calendar size={36} strokeWidth={2.2} className="text-white" /></div>
            ...
            return (
              <div key={apt.id} className="pro-card p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-2 text-sm font-bold text-slate-600 dark:text-slate-350">
                   <span className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-100 dark:border-slate-800"><Clock size={16} strokeWidth={2.2} className="text-cyan-500"/> {apt.slot}</span>
                   <span className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-100 dark:border-slate-800"><IndianRupee size={16} strokeWidth={2.2} className="text-emerald-500"/> {displayAmount(apt.amount)}</span>
                </div>
                ...
                <div className="mt-4 rounded-lg border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-3">
                  ...
                  <div className="mt-4 flex items-start gap-2 text-xs font-bold text-slate-600 dark:text-slate-400">
                    <FileText size={16} strokeWidth={2.2} className="text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
                ...
                {canSubmitPayment && (
                  <div className="mt-6 rounded-lg border border-cyan-100 dark:border-cyan-950/60 bg-cyan-50/70 dark:bg-cyan-950/20 p-4 space-y-3">
                    ...
                    <Button onClick={() => onOpenUpi(apt)} variant="accent" className="w-full text-sm py-3 shadow-none" disabled={!paymentReceiver}>
                      <Wallet size={16} strokeWidth={2.2} /> Open UPI App
                    </Button>
                ...
                {isPaymentVerified(apt) && (
                  <Button onClick={() => generateReceipt(apt, platformFeePercent)} variant="secondary" className="mt-6 w-full text-sm py-3">
                    <FileText size={16} strokeWidth={2.2} /> Download Receipt
                  </Button>
                )}
              </div>
            );
